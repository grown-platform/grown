...
// Collaboration for slides is a simple op broadcast: the editor emits change
// operations (and presence) which we relay to the other clients editing the
// same deck. Durability + late-joiner state come from the autosaved deck
// (SaveDeck / GetDeck), so the hub itself keeps no state — it just broadcasts.
...
func (h *Hub) Serve(w http.ResponseWriter, r *http.Request, deckID string, canWrite bool) {
	...
	for {
		...
		// Read-only viewers may not mutate the deck; relay only presence.
