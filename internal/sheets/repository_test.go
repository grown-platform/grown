...
func TestRepository_CreateRenameSaveTrash(t *testing.T) {
	...
	s, err := repo.Create(ctx, orgID, userID, "Budget")
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
	if _, err := repo.Rename(ctx, orgID, s.ID, "Budget 2026"); err != nil {
		t.Fatalf("Rename: %v", err)
	}
	if err := repo.Save(ctx, orgID, s.ID, `{"cells":{}}`); err != nil {
		t.Fatalf("Save: %v", err)
	}
	got, err := repo.Get(ctx, orgID, s.ID)
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
...
func TestRepository_OrgIsolation(t *testing.T) {
	...
	if got, _ := repo.List(ctx, otherOrg); len(got) != 0 {
		t.Fatalf("cross-org leak: other org saw %d sheets", len(got))
	}
}
